app.controller('appLinkController', ['$scope', '$http', '$window', function($scope, $http, $window) {
	...
	function createTable(items) {
		...
		// array for storing & swapping link positions

		$scope.list = Array(items.length);

		for (var i = 0; i < items.length; i++) {

			$scope.list[i] = items[i].name;
	...
	// function for changing app link order

	$scope.updateOrder = function() {

		$http.post('/setUserAppsOrder',{login: $scope.login, password: $scope.password, appNames: $scope.list})
		...
		.error(function() {
			...
			console.log('Failed to update after drag & drop');
